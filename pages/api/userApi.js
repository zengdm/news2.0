...
userApi.prototype = {
    ...
    apiURL: {
        ...
        //用户统一获得点赞
        userPraise: '/es/comment/supports'
    },
                ...
                wx.getSetting({
                    success: function (resp) {
                        ...
                        if (resp.authSetting['scope.userInfo']) {
                            ...
                            if (url == 'pages/inc/userInfo/userInfo') {
                                return false
                            }

                            // wx.navigateTo({
                            //   url: '/pages/inc/userInfo/userInfo'
                            // })
    ...
    getLoginInfo: function (callback) {
        ...
        if (!loginInfo) {
            // 从缓存中获取
            // console.log(wx.getStorageSync(that.globalKey.login));
   ...
   */
     ...
     * @param [object]  args      [其它查询参数,默认{}]
     */
    addsearch: function (ftype,callback, args) {
        ...
        console.log(args)
        // 点赞只允许一次
        if (ftype == 'search' && isCache) {
            console.log("有缓存")
            // that.callback_page(callback, { code: 200, 'message': '已加入缓存' }, { 'from': 'cache' });
        } else {
            console.log("没有缓存")
        //     // 写入缓存
            // wx.setStorage({
   ...
   * @param [object]  args      [其它查询参数,默认{}]
   */
    addonPraise: function (ftype, sourceid, platid, uid, callback, args) {
        var that = this;
        // 查询参数
        args = typeof (args) != 'object' ? {} : args;
        args['ftype'] = ftype;
        ...
        if (ftype == 'praise' && isCache) {
            ...
            that._addCache(ftype, args);
            // 执行操作
            that.postURLData(that.apiURL['addonPraise'], args, callback);
        }
    },
