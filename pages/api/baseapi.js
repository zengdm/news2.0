...
function baseapi(page) {
  ...
  that.curlData = function (rtype, path, rdata, callback) {
    ...
    if (rtype == 'post' || rtype == 'get') {
      ...
      // 当前程序版本号
      var appVersion = getApp().appVersion;
      // 添加小程序标识
      rdata['appkey'] = getApp().appkey;
      // 小程序分配的平台id
      var fromplatid = getApp().fromplatid;
      // http请求
      wx.request({
        ...
        header: {
          ...
          'appvars': "{ \"appVersion\": \"" + appVersion +"\", \"fromplatid\": \""+fromplatid+"\", \"loginToken\": \"" + loginToken + "\"}",
        },
