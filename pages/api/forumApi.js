...
forumApi.prototype = {
  ...
  apiURL: {
    ...
    //pc、wap小程序统一新增评论
    'addreplyComment': '/es/comment/add',
    //回帖列表 
    'replyList': '/forum/reply/rlist',
    //pc、wap小程序统一回帖列表 
    'replyListcomment': '/es/comment',
    //es feed列表 
    'esFeed': '/es/feed',
    //分享统计代码
    'shareStongji': '/trace/tongji/shares',
  },
  /**
   * 分享统计代码
   ...
   */
  shareStongji: function (sourceid, platid) {
    var that = this;
    var args = {sourceid, platid };
    that.getURLData(that.apiURL['shareStongji'], args);
  },

/**
 * es feed列表
 */
  esFeed: function (page, args, authorid,callback,feedArgs) {
    var that = this;
    feedArgs = typeof (feedArgs) != 'object' ? {} : feedArgs;
    feedArgs['page'] = page;
    feedArgs['args'] = args;
    feedArgs['authorid'] = authorid;
 ...
 */
   ...
   * @param [string]  页面回调方法
   */
  addreplyComment: function (args, callback) {
      var that = this;
      that.getURLData(that.apiURL['addreplyComment'], args, callback);
  },
...
*/
  ...
  /**
* pc、wap小程序统一我的帖子列表
* @param [Object]  args  [翻页参数]
* @param [string]  callback  [页面回调方法]
*/

  replyListcomment: function (args,callback) {
      var that = this;
      // 异步获取接口数据
      that.postURLData(that.apiURL['replyListcomment'], args, callback);
  },
