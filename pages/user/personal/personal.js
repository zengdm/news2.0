...
Page({
  ...
  onLoad: function (options) {
    ...
    // 4. 获取feed流列表
    that.myforum(that.data.nextPage);
    // 设置模板变量
    that.setGlobalData();
  ...
  cb_esFeed: function (res, opt) {
    ...
    if (res.code == 0) {
      ...
      if (len > 0) {
        ...
        // 记录分页数据
        if (!that.data.myforum['pageRecord']) {
          that.data.myforum['pageRecord'] = 1;
        }
        that.data.myforum['pageRecord'] = nextPage;

        that.data.myforum['isEnd'] = false;
        that.setData({
          forumList: that.data.myforum
        })
      } else {
 ...
 */
  onPullDownRefresh: function () {
    ...
    var that = this;
    var page = that.data.myforum['pageRecord'];
